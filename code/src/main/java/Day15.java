...
public record Day15(int part1, int part2) {
    ...
    public static Day15 fromSeeds(int seedA, int seedB) {
        return new Day15(
                countMatches(
                        new Generator(seedA, FACTOR_A, 1),
                        new Generator(seedB, FACTOR_B, 1),
                        40_000_000),
                countMatches(
                        new Generator(seedA, FACTOR_A, 4),
                        new Generator(seedB, FACTOR_B, 8),
                        5_000_000));
    }
        ...
        return (int)
                IntStream.range(0, times)
                        .filter(
                                i ->
                                        compareTrailingBits(
                                                genA.getAndGenerateNext(),
                                                genB.getAndGenerateNext()))
                        .count();
    }
